...
def cornersHeuristic(state, problem):
    ...
    return max(map(lambda corner: (abs(state[0][0] - corner[0]) + abs(state[0][1] - corner[1])), state[1]))
...
def foodHeuristic(state, problem):
    ...
    if problem.isGoalState(state):
        return 0
    return max(map(lambda food: (abs(position[0] - food[0]) + abs(position[1] - food[1])), foodGrid.asList()))
